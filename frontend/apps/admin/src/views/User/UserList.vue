  ...
  <div class="user-list-container">
    <div class="search-bar">
      <el-form :inline="true" :model="searchForm">
        <el-form-item label="关键词">
          <el-input v-model="searchForm.keyword" placeholder="用户名/邮箱/学校" clearable @keyup.enter="handleSearch" />
        </el-form-item>
    ...
    <el-table v-loading="loading" :data="userList" border style="width: 100%">
      ...
      <el-table-column label="操作" width="200" fixed="right">
        <template #default="{ row }">
          <el-button-group>
            <el-button v-if="!row.status" type="danger" size="small" @click="handleBan(row)">
              封禁
            </el-button>
    ...
    <el-dialog v-model="banDialogVisible" title="封禁用户" width="400px">
      <el-form :model="banForm" label-width="100px">
        <el-form-item label="封禁结束时间">
          <el-date-picker v-model="banForm.endTime" type="datetime" placeholder="选择日期时间" format="YYYY-MM-DD HH:mm:ss"
            :min-date="new Date()" />
        </el-form-item>
      </el-form>
      <template #footer>
