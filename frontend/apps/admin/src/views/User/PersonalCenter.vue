...
onMounted(async () => {
  await userApi.getInfoById(id.value).then(res => {
    const { code, info } = res
    if (code == 200 && info) {
      const { id, username, nickname, gender, avatar, rating, codeforces, created_at,submission, accept, email, introduction, school, status, banned_to, balance} = info
      userForm.id = id
      ...
      userForm.codeforces = codeforces
      userForm.created_at = created_at
      userForm.submission = submission
      userForm.accept = accept
      userForm.email = email
      userForm.introduction = introduction
      userForm.school = school
      userForm.status = status
      userForm.banned_to = banned_to
        ...
        <el-col :span="18">
          <el-form ref="formRef" :model="userForm" :rules="rules" label-width="100px" class="user-form">
            ...
            <el-form-item label="个人简介" prop="bio">
              <el-input v-model="userForm.introduction" type="textarea" :rows="3" placeholder="请输入个人简介" />
            </el-form-item>
          </el-form>
        </el-col>
      </el-row>
