    ...
    <div class="relative w-full max-w-md rounded-2xl p-8 border backdrop-blur-xl" :style="{
      ...
      <n-form ref="formRef" :model="modelRef" :rules="rules" class="space-y-5">
        ...
        <n-form-item path="username" :show-label="false">
          <n-input v-model:value="modelRef.username" placeholder="请输入用户名" size="large" @keydown.enter.prevent>
            <template #prefix>
        ...
        <n-form-item path="password" :show-label="false">
          <n-input v-model:value="modelRef.password" type="password" placeholder="请输入密码" size="large"
            show-password-on="click" @input="handlePasswordInput" @keydown.enter.prevent>
            <template #prefix>
        ...
        <n-form-item ref="rPasswordFormItemRef" first path="reenteredPassword" :show-label="false">
          <n-input v-model:value="modelRef.reenteredPassword" :disabled="!modelRef.password" type="password"
            placeholder="请再次输入密码" size="large" show-password-on="click" @keydown.enter.prevent>
            <template #prefix>
              <Lock :size="18" :style="{ color: 'var(--text-tertiary)' }" />
            </template>
          </n-input>
        ...
        <!-- 登录按钮 -->
        <div class="pt-2">
          <n-button :disabled="modelRef.username === null" type="primary" size="large" block strong
            @click="handleValidateButtonClick" :style="{
              height: '48px',
              fontSize: '16px',
              fontWeight: '600',
              borderRadius: '12px',
