                ...
                <div class="flex flex-col h-full">
                        ...
                        <div v-if="currentChatFriend" class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="text-lg font-semibold" :style="{ color: 'var(--text-primary)' }">
                                    {{ currentChatFriend.remark || currentChatFriend.friend_nickname ||
                                        currentChatFriend.friend_username }}
                                </h2>
                                <p v-if="currentChatFriend.remark" class="text-xs"
                    ...
                    <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-4">
                        ...
                        <div ref="loadMoreTrigger" class="flex justify-center py-2">
                            ...
                            <p v-else-if="!hasMoreMessages && chatMessages.length > 0" class="text-xs"
                                :style="{ color: 'var(--text-tertiary)' }">
                                没有更多消息了
                            </p>
                        </div>
                        ...
                        <n-spin :show="messagesLoading">
                            <div v-if="currentChatFriendId && chatMessages.length > 0" class="space-y-4">
                                <div v-for="msg in chatMessages" :key="msg.id"
                                    ...
                                    <div
                                        :class="['flex max-w-75%', isCurrentUser(msg) ? 'flex-row-reverse' : 'flex-row']">
                                        <!-- 头像 -->
                                        <n-avatar :size="40"
                                            :src="isCurrentUser(msg) ? avatar : currentChatFriend?.friend_avatar || undefined"
                                            class="shrink-0">
                    ...
                    <div class="p-4 border-t" :style="{ borderColor: 'var(--border-color)' }">
                        <div class="flex gap-3">
                            <n-input v-model:value="MessageContent" type="textarea" placeholder="输入消息..."
                                :autosize="{ minRows: 1, maxRows: 4 }" :disabled="!currentChatFriendId" />
                            <n-button type="primary" @click.stop="handleSend" :disabled="!currentChatFriendId">
                                发送
                            </n-button>
                        </div>
                    </div>
...
import type { FriendShip, FriendShipRequest } from '@nexusoj/type'
...
import { useNexusWebSocket } from '@/composables/useWebSocket'
...
const loadMoreMessages = async () => {
    ...
    try {
        ...
        // 后端返回倒序（该页最新的在前），需要反转后插入到列表前面（因为是更旧的消息）
...
const startChat = async (friend_id: Number) => {
    ...
    try {
        ...
        // 标记已读 - 更新 friendship 的 unread_count
        const friendship = friendList.value.find(f => f.friend_id === friend_id)
        if (friendship && friendship.unread_count > 0) {
            await userApi.MarkMessagesAsRead(friend_id)
...
const { send, status, open, close } = useNexusWebSocket(
    `/ws/chat?token=Bearer ${AccessToken.value}`,
    {
        onOpen() {
        ...
        onMessage(msg) {
            ...
            // 解析消息（可能是字符串或对象）
            const chatMessage: ChatMessage = typeof msg.data === 'string'
                ? JSON.parse(msg.data)
                : msg.data

            const { sender_id, receiver_id, content } = chatMessage

            console.log('Received message:', chatMessage)

            // 判断消息来自好友还是自己发送的
            const friendId = sender_id === id ? receiver_id : sender_id

            // 查找对应的好友关系
            const friendship = friendList.value.find(f => f.friend_id === friendId)

            if (friendship) {
                // 更新最新消息
                friendship.latest_message = content || ''

                // 如果不是当前聊天，增加未读计数
                if (currentChatFriendId.value !== friendId) {
                    friendship.unread_count = (friendship.unread_count || 0) + 1
                } else {
                    // 如果是当前聊天，添加到 UI 并滚动到底部
...
const handleSend = () => {
    ...
    if (msg.content.trim() === '' || !msg.receiver_id || !msg.sender_id) {
        message.error('不能发送空消息')
        return
    }

    // 发送消息，不立即添加到缓存
    // 等待 WebSocket 收到服务器确认的消息后再处理
...
const handleScroll = (event: Event) => {
    ...
    if (target.scrollTop === 0 && !isLoadingMore.value && hasMoreMessages.value) {
        loadMoreMessages()
    }
}
